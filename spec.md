...
One GitHub Action step. Isolated D1 databases and KV namespaces per pull request. Preview versions of your existing Workers with stable aliased URLs. Automatic cleanup on merge/close.

---

## Overview

`cf-preview-stack` is a GitHub Action that gives Cloudflare Workers projects the same preview environment experience that Vercel and Netlify provide out of the box. For every pull request, it provisions isolated D1 databases and KV namespaces, runs migrations, rewrites wrangler configs with preview resource IDs, and uploads a preview version of each Worker using Cloudflare's native versioning system — then cleans up the resources when the PR closes.

It works *with* the Cloudflare platform, not against it. Worker previews use `wrangler versions upload --preview-alias`, which is Cloudflare's built-in mechanism for per-branch preview URLs. The action fills the gap that Cloudflare doesn't solve: the *stateful resource lifecycle* — creating and destroying isolated D1 databases and KV namespaces (and eventually R2 buckets) per PR so each preview version runs against its own data.
...
1. Creates ephemeral D1 databases and KV namespaces, and runs migrations against D1.
2. Rewrites wrangler configs to bind to the preview resources.
3. Calls `wrangler versions upload --preview-alias pr-{N}` to create a preview version that uses the isolated data.

The result: your production Worker is untouched. The preview version runs at a stable URL like `pr-42-api.your-subdomain.workers.dev`, backed by its own database and KV namespaces. When the PR closes, only the ephemeral resources are deleted — the preview version simply becomes stale and is eventually garbage-collected by Cloudflare.
...
| `kv_namespace_ids` | JSON object mapping original KV namespace ID → preview namespace ID. |
...
Preview KV namespaces use the pattern `preview-pr-{number}-{binding_name}`.

Example: PR #42 with a KV binding named `MY_KV` → namespace titled `preview-pr-42-MY_KV`.

If two workers share the same KV namespace `id`, only one preview namespace is created and both workers' configs are rewritten to point to it. KV namespaces start empty — there is no migration or seed data concept for KV.
...
1. TEARDOWN RESOURCES
   - List all D1 databases matching `preview-pr-{N}-*` via Cloudflare SDK — delete each one
   - List all KV namespaces matching `preview-pr-{N}-*` via Cloudflare SDK — delete each one
...
2. PROVISION RESOURCES
   For each worker config:
     ...
     - For each KV namespace binding:
       - Create namespace `preview-pr-{N}-{binding_name}` via Cloudflare SDK
       - Record the new namespace id
     - Rewrite wrangler.jsonc on the runner's filesystem:
       - Replace each D1 `database_id` with the preview database ID
       - Replace each D1 `database_name` with the preview database name
       - Replace each KV `id` with the preview namespace ID
...
5. COMMENT
   - Post or update a PR comment with preview URLs, database info, and KV namespace info
```
...
1. TEARDOWN RESOURCES
   ...
   (No worker cleanup needed — preview versions are inert once
    their resources are gone, and Cloudflare garbage-collects
...
| KV Namespace | Binding | Preview Title |
|--------------|---------|---------------|
| MY_KV | ns-original-id | preview-pr-42-MY_KV |
...
| `cloudflare` | Cloudflare TypeScript SDK — D1 database and KV namespace create/delete/list |
...
Teardown handles D1 databases and KV namespaces. Worker versions are managed by Cloudflare and don't need cleanup.
...
// List all KV namespaces, filter by prefix
const allKV = await cf.kv.namespaces.list({ account_id });
const previewKV = allKV.filter(ns =>
  ns.title.startsWith(`preview-pr-${prNumber}-`)
);
for (const ns of previewKV) {
  await cf.kv.namespaces.delete(ns.id, { account_id });
}
```
...
| Account Settings: Read | Account | Listing D1 databases and KV namespaces |
...
- **D1 and KV only.** R2 buckets are not provisioned. If your worker uses R2, the preview version will bind to whatever is in the existing config (typically production resources). Use wrangler environments or environment variables to point at shared dev instances.
- **KV namespaces start empty.** There is no seed data or cloning mechanism for KV. Preview namespaces are created fresh and empty on every push.
...
│   ├── kv.ts               # KV namespace create/delete/list via Cloudflare SDK
